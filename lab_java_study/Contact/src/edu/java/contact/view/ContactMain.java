...
public class ContactMain {
	...
	public void notifyContactSearch() {
		
		JOptionPane.showMessageDialog(frame, "연락처 검색 완료");
	...
	private void loadContactData() {
		...
		contactList = (List<Contact>) dao.read();

		for (int i = 0; i < contactList.size(); i++) {
			Object[] rowdata = { i + 1 + "", contactList.get(i).getName(), contactList.get(i).getPhone(),
	...
	private void initialize() {
		...
		btnInsert.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				ContactCreateFrame.showFrame(frame, ContactMain.this);
	...
	protected void handleclickbtnSearch() {
		...
		ContactSearchFrame.showSearchFrame();
	...
	protected void handleclickbtnModify() {
		...
		ContactUpdateFrame.showContactUpdateFrame(frame, select[0], ContactMain.this, contactList);
	...
	protected void handleclickbtnDelete() {
		...
		if (selectedRows.length >= 1) {
			for (int i = 0; i < selectedRows.length; i++) {
				dao.delete(contactList.get(selectedRows[i]).getCid());
