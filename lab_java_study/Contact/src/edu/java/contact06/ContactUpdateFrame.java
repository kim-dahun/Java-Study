...
public class ContactUpdateFrame extends JFrame {
	...
	private List<Contact> list; // 업데이트할 연락처의 인덱스를 저장하기 위한 필드
	...
	public static void showContactUpdateFrame(Component parent, List<Contact> list, ContactMain06 app, int index) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ContactUpdateFrame frame = new ContactUpdateFrame(parent, list, app, index);
	...
	private ContactUpdateFrame(Component parent, List<Contact> list , ContactMain06 app, int index) {
		this.index = index;
		this.list = list;
		this.parent = parent;
		this.app = app;
